...
__export(schema_exports, {
  ...
  hostCapabilityManifestSchema: () => hostCapabilityManifestSchema,
...
var componentConfigFieldSchema = z.object({
   ...
   *
   * The runtime declares ConfigField, catalog/manifests.ts declared a
   * structural duplicate as ManifestConfigField, and this zod enum validates
   * what is published. Adding "any" to the runtime's union on 17 Aug fixed
   * the second by aliasing it and missed this one — so `symbia.logic.filter`
   * registered its manifest, was refused with `400: Invalid component
   * manifest`, and the component was simply absent from the catalog. Found
   * by booting the stack, not by reading it.
   *
   * "any" means the component compares or forwards the value without caring
   * what it is. filter's `value` is genuinely untyped — it is tested against
   * whatever the message holds, with `===` — and declaring it `string` was
...
var resources = pgTable("resources", {
  ...
  /**
   * POSITION, so ordering does not rest on a clock.
   *
   * Assigned by the database at insert, so it is a place in commit order
   * rather than a reading. `created_at` is a wall-clock value and therefore
   * apocryphal — nothing in the record recomputes it — which made "was this
   * registered before that" a question the catalog could only answer with a
   * value it could not defend. See
   * docs/proposals/2026-08-20-tick-spine-and-placement.md.
   *
   * Exact within this catalog. It says nothing about ordering against another
   * chain; that needs both events placed against their anchors and compared as
   * intervals, and is only as good as those anchors.
   */
  writeSeq: bigint("write_seq", { mode: "number" }),
  /**
   * Measured duration of the request that wrote this row.
   *
   * `symbia-http` has computed this on every request since it shipped
   * (telemetry.ts:24, logging.ts:100) and discarded it before the row. Placement
   * needs it so that only the unmeasured residual is distributed instead of
   * interpolating uniformly, which probe T4 measured going worst at the largest
   * idle gap.
   */
  writeMs: integer("write_ms"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull()
}, (table) => ({
  typeIdx: index("idx_resources_type").on(table.type),
  writeSeqIdx: index("idx_resources_write_seq").on(table.writeSeq),
...
var DatabaseStorage = class {
  ...
  /**
   * NOT the same question as `createdAt`.
   *
   * `write_seq` is assigned by the database and needs no help here. `write_ms`
   * is the duration of the work, which only the caller can time, so it is
   * passed in rather than measured inside a method that has already started.
   */
...
function createRateLimiter(name, getWindowMs2, getMaxRequests, message, getServiceMaxRequests) {
  ...
  return (req, res, next) => {
    ...
    const service = getServiceMaxRequests !== void 0 && isInternalService(req);
    const key = (service ? "service:" : "caller:") + (req.user?.id || req.ip || "anonymous");
    const now = Date.now();
    const windowMs = getWindowMs2();
    const maxRequests = service ? getServiceMaxRequests() : getMaxRequests();
...
var getServiceWriteMax = () => parseInt(process.env.RATE_LIMIT_SERVICE_WRITE_MAX || "300", 10);
...
var RATE_LIMITS = {
  ...
  get serviceWriteMaxRequests() {
    return getServiceWriteMax();
  },
...
var DEFAULT_CONFIG = {
  ...
  allowedMimeTypes: [
    ...
    "application/x-tar",
    "text/plain",
    // 20 Aug 2026: a markdown publication was refused here and had to be
    // uploaded as text/plain with a .txt suffix appended to its name. The
    // allowlist exists to keep executable and unexpected content out; markdown
    // is neither, and it is the format the platform's own documents are in.
    "text/markdown",
    "text/html",
    "text/css",
    "text/csv",
...
var updateResourceSchema = z2.object({
   ...
   *
   * Making create strict and leaving update permissive would fix the loud half
   * and keep the quiet one: a caller correcting a resource is exactly as able to
   * send a field this schema does not know, and would get the same 200 over the
   * same silent drop. Worse on update, in fact — a caller patching a record
   * usually believes it already exists and is being repaired, so a dropped field
   * reads as "the correction was applied" rather than "the write was ignored".
   *
   * Same reasoning as create and as the graph schema below: reject and name the
...
var createResourceSchema = z2.object({
   ...
   *
   * The graph schema below carries a careful note about this exact defect,
   * measured 8 Aug: a plain `z.object` strips unknown keys during `.parse()`, so
   * a caller sends a field, gets 201, and the field is gone. That lesson was
   * applied to `/api/graphs` and not to `/api/resources` beside it.
   *
   * MEASURED 24 Aug, on this endpoint, by an agent recording its own work:
   * sixteen `POST /api/resources` calls carrying a top-level `data` object all
   * returned 200 with a resource body, and all sixteen persisted as a title, a
   * type and tags with nothing inside. `resources` has a `metadata` jsonb column
   * and no `data` column. The response never contained `data` either, which was
   * the evidence available from the first call.
   *
   * The records in question were predictions and findings — a provenance ledger
   * that was empty exactly where the content should be, and complete-looking from
   * outside. Recorded as F53.
   *
   * Rejecting rather than quietly folding unknown keys into `metadata`, for the
   * reason the graph note already gives: guessing what a field means replaces a
   * silent drop with a silent reinterpretation. A 400 naming the field ends it in
   * one round trip.
   */
}).strict();
...
async function checkMapReferences(metadata, tags, lookup) {
  const problems = [];
  if (!(tags ?? []).includes("map")) return problems;
  const raw = metadata?.references;
  const refs = (typeof raw === "string" ? [raw] : Array.isArray(raw) ? raw : []).filter(
    (r) => typeof r === "string" && r.length > 0
  );
  if (!refs.length) return problems;
  for (const ref of refs) {
    const target = await lookup(ref);
    if (!target) {
      problems.push({
        reference: ref,
        problem: "no resource with this key exists. A results resource citing a prediction that is not in the catalog documents an ordering it cannot demonstrate"
      });
    }
  }
  return problems;
}
var GATED_KEY_PREFIXES = [
  { prefix: "models/", type: "model" },
  { prefix: "documents/", type: "document" },
  { prefix: "host-capabilities/", type: "host-capability" }
];
function checkKeyPrefixTypeAgreement(key, type) {
  for (const gate of GATED_KEY_PREFIXES) {
    if (key.startsWith(gate.prefix) !== (type === gate.type)) {
      return `key-prefix and type disagree: keys under ${gate.prefix} must have type '${gate.type}', and type '${gate.type}' requires a ${gate.prefix} key`;
    }
  }
  return null;
}
async function registerRoutes(httpServer, app) {
  ...
  app.get("/api/resources", authMiddleware, searchRateLimiter, async (req, res) => {
    try {
      ...
      const withheld = allResources.length - filterResourcesByReadAccess(allResources, req.user).length;
      res.setHeader("X-Symbia-Total", String(allResources.length));
      res.setHeader("X-Symbia-Withheld", String(withheld));
      if (withheld > 0) {
        res.setHeader(
          "X-Symbia-Withheld-Reason",
          "read access policy; authenticate or use a principal with broader entitlements to see them"
        );
      }
  ...
  app.post("/api/resources", authMiddleware, writeRateLimiter, async (req, res) => {
    ...
    try {
      ...
      const keyProblem = checkKeyPrefixTypeAgreement(validatedData.key, validatedData.type);
      if (keyProblem) {
        return res.status(400).json({ error: keyProblem });
      }
      if (validatedData.key.startsWith("models/") && !/^models\/[a-z0-9][\w.-]*\/.+$/.test(validatedData.key)) {
        return res.status(400).json({
          error: "model keys are models/<publisher>/<name>: publisher segment required (use 'local' when there is no upstream)"
        });
      }
      if (validatedData.type === "host-capability") {
        const rawHC = validatedData.metadata ?? {};
        const hcInput = rawHC.manifest ?? rawHC;
        const hcManifest = hostCapabilityManifestSchema.safeParse(hcInput);
        if (!hcManifest.success) {
          return res.status(400).json({
            error: "Invalid host-capability manifest",
            details: hcManifest.error.errors
          });
        }
        validatedData.metadata = { ...rawHC, manifest: hcManifest.data };
      }
      ...
      {
        const refProblems = await checkMapReferences(
          validatedData.metadata,
          validatedData.tags,
          (k) => storage.getResourceByKey(k)
        );
        if (refProblems.length) {
          return res.status(409).json({
            error: "map references do not resolve",
            problems: refProblems,
            note: "Register the predictions first, then record results citing them. Omit `metadata.references` entirely if this measurement had no registered prediction."
          });
        }
      }
      ...
      const resource = await storage.createResource({
        ...
        writeMs: Math.round(Number(process.hrtime.bigint() - t0) / 1e6)
      });
  ...
  app.patch("/api/resources/:id", authMiddleware, writeRateLimiter, async (req, res) => {
    try {
      ...
      {
        const keyProblem = checkKeyPrefixTypeAgreement(
          validatedData.key ?? resource.key,
          validatedData.type ?? resource.type
        );
        if (keyProblem) {
          return res.status(400).json({ error: keyProblem });
        }
      ...
      if (validatedData.metadata !== void 0 && validatedData.metadata !== null) {
        ...
        if (effectiveType === "host-capability") {
          const manifest = hostCapabilityManifestSchema.safeParse(manifestInput);
          if (!manifest.success) {
            return res.status(400).json({
              error: "Invalid host-capability manifest",
              details: manifest.error.errors
  ...
  app.post("/api/graphs", authMiddleware, writeRateLimiter, async (req, res) => {
    try {
      ...
      const graph = await storage.createResource({
        ...resourceData,
        createdBy: req.user?.id ?? null
      });
  ...
  app.post("/api/contexts", authMiddleware, writeRateLimiter, async (req, res) => {
    ...
    try {
      ...
      {
        const keyProblem = checkKeyPrefixTypeAgreement(validatedData.key, "context");
        if (keyProblem) {
          return res.status(400).json({ error: keyProblem });
        }
      }
      ...
      let accessPolicy;
      if (validatedData.accessPolicy) {
        accessPolicy = {
          ...defaultAccessPolicy,
          ...validatedData.accessPolicy,
          actions: {
            ...defaultAccessPolicy.actions,
            ...validatedData.accessPolicy.actions ?? {}
          }
        };
      } else if (validatedData.orgId) {
        accessPolicy = {
          visibility: "org",
          actions: {
            read: { anyOf: [`org:${validatedData.orgId}`, `role:member:${validatedData.orgId}`, `role:admin:${validatedData.orgId}`] },
      ...
      const context = await storage.createResource({
        ...resourceData,
        createdBy: req.user?.id ?? null,
        writeMs: Date.now() - t0
      });
