...
async function initDatabase() {
  ...
  try {
    ...
    await client.query(`ALTER TABLE participants ADD COLUMN IF NOT EXISTS capacity VARCHAR(120)`);
    await client.query(`
...
var init_conversation = __esm({
  "../messaging/server/src/models/conversation.ts"() {
    ...
    ConversationModel = {
       ...
       */
      /**
       * Find the conversation for a chat room.
       *
       * connectionId IS OPTIONAL, AND USED TO BE REQUIRED.
       *
       * A connectionId is minted fresh by every connect — a restart, a supervisor
       * reconnect, a token refresh. Requiring it here meant the lookup could only
       * ever find conversations belonging to the CURRENT socket, so after any
       * reconnect the caller found nothing and created a new conversation.
       *
       * Measured 24 Aug: thirteen conversations for one Twitch channel, and an
       * assistant that answered every message with no memory of the one before.
       *
       * The room is the durable identity. Newest first, because a stack that has
       * already accumulated duplicates should converge on the one still in use
       * rather than resurrect the oldest.
       */
      async findByChannelMetadata(channelType, connectionId, chatId) {
        const params = [channelType, chatId];
        let connectionFilter = "";
        if (connectionId) {
          params.push(connectionId);
          connectionFilter = `AND metadata->'channel'->>'connectionId' = $3`;
        }
        const result = await pool.query(
          `SELECT * FROM conversations
       WHERE metadata->'channel'->>'type' = $1
         AND metadata->'channel'->>'chatId' = $2
         ${connectionFilter}
...
var ParticipantModel = {
   ...
   */
  async add(conversationId, userId, userType, role = "member", entityId, capacity) {
    const resolvedUserType = userType ?? (userId.startsWith("assistant:") || userId.startsWith("agent:") ? "agent" : "user");
    const result = await pool.query(
      `INSERT INTO participants (conversation_id, user_id, user_type, role, entity_id, capacity)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (conversation_id, user_id) DO UPDATE SET
         role = EXCLUDED.role,
         entity_id = COALESCE(EXCLUDED.entity_id, participants.entity_id),
         -- COALESCE so re-adding somebody without stating a capacity does not
         -- erase the one they were given. Silently blanking it would be the
         -- worst outcome: the room would look like it never knew.
         capacity = COALESCE(EXCLUDED.capacity, participants.capacity)
       RETURNING *`,
      [conversationId, userId, resolvedUserType, role, entityId, capacity ?? null]
    );
    return result.rows[0];
  },
  ...
  async listForConversation(conversationId, options = {}) {
    ...
    const result = await pool.query(
      `SELECT * FROM (
         ...
         ORDER BY sequence DESC NULLS LAST, created_at DESC LIMIT $2
       ) recent
...
router2.post("/", requireAuth, async (req, res) => {
  try {
    ...
    await ParticipantModel.add(
      conversation.id,
      req.user.id,
      req.user.type,
      "owner",
      void 0,
      typeof req.body?.capacity === "string" ? req.body.capacity : void 0
    );
      ...
      for (const p of participants) {
        if (p.userId && p.userId !== req.user.id) {
          await ParticipantModel.add(
            ...
            "member",
            void 0,
            typeof p.capacity === "string" ? p.capacity : void 0
          );
        }
      }
    }
...
router2.post("/:id/participants", requireAuth, async (req, res) => {
  try {
    ...
    const participant = await ParticipantModel.add(
      id,
      userId,
      userType,
      "member",
...
async function registerRoutes(_server, app) {
  ...
  app.get("/api/internal/conversations/by-channel", async (req, res) => {
    ...
    if (!channelType || !chatId) {
      res.status(400).json({ error: "channelType and chatId query params required" });
      return;
    }
    try {
