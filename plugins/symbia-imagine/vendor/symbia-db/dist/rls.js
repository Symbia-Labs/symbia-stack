...
function canBypassOrgFilter(context) {
    ...
    if (context.capabilities) {
        return context.capabilities.some((cap) => GLOBAL_READ_CAPABILITIES.includes(cap));
    }
    return false;
}
/**
 ...
 */
export async function setSessionContext(client, context) {
 ...
 * @param client - Database pool or client
 */
export async function clearSessionContext(client) {
    await client.query(`
    SELECT
 ...
 */
export async function withRLSContext(pool, context, fn) {
 ...
 */
export function generateRLSPolicies(tableName, options = {}) {
    const { orgIdColumn = "org_id", nullableOrgId = false, } = options;
 ...
 * @param tableName - Name of the table
 */
export function generatePublicRLSPolicy(tableName) {
    return `
 ...
 * @param tableName - Name of the table
 */
export function generateDropRLSPolicies(tableName) {
    return `
 ...
 */
export function generateRLSMigration(tables) {
    const policies = Object.entries(tables).map(([table, options]) => generateRLSPolicies(table, options));
    return `
