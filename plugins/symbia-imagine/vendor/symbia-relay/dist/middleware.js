...
export function observabilityMiddleware(options = {}) {
    const { excludePaths = ['/health', '/health/live', '/health/ready', '/favicon.ico'], excludePatterns = [], includeHeaders = false, excludeHeaders = DEFAULT_EXCLUDE_HEADERS, slowRequestThresholdMs = 5000, traceIdHeader = 'x-trace-id', } = options;
    return (req, res, next) => {
        // Check exclusions
        ...
        const traceId = req.headers[traceIdHeader] ||
            traceIdFromHeaders(req.headers) ||
            mintTraceId();
