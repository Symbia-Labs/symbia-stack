 ...
 */
export function resolveVaultSecret(env = process.env) {
    ...
    if (!warnedDevFallback) {
        ...
        console.warn("[symbia-crypto] CREDENTIAL_ENCRYPTION_KEY unset — using dev-only vault key. " +
...
export function deriveVaultKey(secret) {
...
export function encryptSecret(plaintext, secret = resolveVaultSecret()) {
...
export function isLegacyCiphertext(stored) {
 ...
 */
export function decryptSecret(stored, secret = resolveVaultSecret(), env = process.env) {
