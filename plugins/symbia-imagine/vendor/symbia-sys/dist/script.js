 ...
 */
export function containsRefs(str) {
...
function resolveAssistantRef(segments, ctx) {
    ...
    if (!registry) {
        return {
            success: false,
            error: 'No assistant registry in this context. It is injected by the assistants service; ' +
    ...
    const found = registry.find((a) => a.alias?.toLowerCase() === needle ||
...
function interpolateEach(template, ctx) {
    return template.replace(EACH_BLOCK, (_match, rawPath, body) => {
        ...
        if (!Array.isArray(list) || list.length === 0)
            return '';
        return list
            .map((item, index) => body.replace(INTERPOLATION_PATTERN, (_m, content) => {
 ...
 */
export function interpolateObject(obj, ctx) {
    const result = {};
 ...
 */
export function getRefSuggestions(partial, ctx) {
    const suggestions = [];
...
export function validateRef(ref) {
    ...
    if ((parsed.namespace === SymbiaNamespace.SERVICE ||
