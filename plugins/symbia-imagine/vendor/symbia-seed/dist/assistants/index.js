...
import { DEFAULT_AGENT_IDS, DEFAULT_ORG_IDS, } from "../shared/constants.js";
 ...
 */
export function generateDefaultAgents() {
 ...
 */
export function generateDefaultPromptGraphs() {
    const now = getSeedTimestamp();
    return [
        {
            id: randomUUID(),
 ...
 * Seed agents
 */
export async function seedAgents(db, agentsTable, config = {}) {
 ...
 * Seed graphs
 */
export async function seedGraphs(db, graphsTable, config = {}) {
    const logger = new SeedLogger(config.verbose);
    try {
 ...
 */
export async function seedAssistantsData(db, schema, config = {}) {
    const logger = new SeedLogger(config.verbose);
