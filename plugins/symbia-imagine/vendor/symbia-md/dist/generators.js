 ...
 */
export function generateLlmsShort(spec, config) {
    const lines = [];
    ...
    lines.push("");
    lines.push(`Use ${spec.servers?.[0]?.url || "/api"} as the base path for all endpoints.`);
    lines.push("");
 ...
 */
export function generateLlmsFull(spec, config) {
    const lines = [];
            ...
            if (details.parameters?.length) {
                ...
                if (queryParams.length) {
                    ...
                    for (const p of queryParams) {
                        lines.push(`- ${p.name} (optional): ${p.description || p.schema?.type || "string"}`);
                    }
                    lines.push("");
                }
            }
