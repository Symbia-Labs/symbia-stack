...
const INCLUDE_GRAPHS = argv.includes('--include-graphs');

function log(message, ...args) {
  console.log(`[bootstrap-pack] ${message}`, ...args);
}

function verbose(message, ...args) {
  if (VERBOSE) console.log(`[bootstrap-pack:debug] ${message}`, ...args);
}
...
function shouldProcess(resource) {
  if (resource.type === 'component' || resource.type === 'context') return true;
  if (INCLUDE_GRAPHS && resource.type === 'graph') return true;
  return false;
}

function desiredBootstrap(resource) {
  if (resource.type === 'graph') return false;
  const pack = getPack(resource.tags);
  return pack === 'pack:Core';
}

async function main() {
  log('Starting bootstrap set update (pack-based)');
  log(`Target: ${OBJECT_SERVICE_URL}`);
  log(`Auth: ${OBJECT_SERVICE_API_KEY ? 'API key configured' : 'No API key (set OBJECT_SERVICE_API_KEY)'}`);
  log(`Dry run: ${DRY_RUN}`);
  log(`Include graphs: ${INCLUDE_GRAPHS}`);
  ...
  for (const resource of resources) {
    if (!shouldProcess(resource)) continue;
    const target = desiredBootstrap(resource);
    if (resource.isBootstrap === target) {
      stats.skipped += 1;
      continue;
    }
    try {
      log(`Updating isBootstrap for ${resource.key}: ${resource.isBootstrap} -> ${target}`);
      await patchResource(resource.id, { isBootstrap: target });
      stats.updated += 1;
      if (DELAY_MS > 0) await sleep(DELAY_MS);
    } catch (err) {
      stats.errors += 1;
      log(`Bootstrap update failed ${resource.key}: ${err.message || err}`);
    }
  }

  log('Bootstrap update complete');
...
main().catch((err) => {
  console.error(`[bootstrap-pack] Fatal: ${err.message || err}`);
  process.exit(1);
});
